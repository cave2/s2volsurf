...
void usage(char *exename) {
  fprintf(stderr, "usage: %s [options] -f infileformat.dbl -o xrwfilename -w gridwidth -h gridheight\n", exename);
...
int main(int argc, char *argv[]) {
  ...
  while (ic < argc) {
	...
	exit(-1);
      ...
      gridwidth = atoi(argv[++ic]);
    } else if (!strcmp(argv[ic], "-H")) {
      gridheight = atoi(argv[++ic]);
    }
    ic++;
  }
  ...
  if (gridwidth < 0 || gridheight < 1) {
    usage(argv[0]);
  }

  fprintf(stderr, "gridwidth, gridheight = %d x %d\n", gridwidth, gridheight);
  ...
  fprintf(stderr, "File index appears to go from %d to %d, step=%d\n", istart, iend, read_stride);
  
  VOL_STRUCT *tgavol = loadPlutoDblStack(ifname, istart, iend, read_stride, gridwidth, gridheight);
  tgavol->wdx = wdelta[0];
  tgavol->wdy = wdelta[1];
  tgavol->wdz = wdelta[2];
